...
def parse_dmarc(directory, files, domain, db, time):
    ...
    try:
        cursor.execute(sql, (dmarc.valid, dmarc.dmarc_policy, dmarc.domain, "Alexa", time))
        db.commit()
    except Exception as e:
...
def parse_mta(directory, files, domain, db, time):
    ...
    for file in files:
        ...
        try:
            cursor.execute(sql, (mta.domain, mta.policy_mode, mx, mta.max_age, "Alexa", time))
            db.commit()
        except Exception as e:
...
def parse_spf(directory, files, domain, db, time):
    ...
    for file in files:
        ...
        if spf.valid == 1:
            ...
            try:
                cursor.execute(sql, (domain, spf.spf_policy, spf.valid, spf.result_description, "Alexa", time))
    ...
    # Invalid SPF Record
    if valid == False:
        cursor.execute(sql, (domain, "none", 0, result_description, "Alexa", time))
        db.commit()
...
if __name__ == '__main__':
    logging.basicConfig(filename="crawling_alexa.log", level=logging.INFO,
    ...
    data = pandas.read_csv("top-1m.csv")
    ...
    # Perform evaluation & loading into database
    ...
    # Create Archive
    archive_name = os.path.join('../results/' , 'alexa_' + crawling_date + '.tar.gz')
